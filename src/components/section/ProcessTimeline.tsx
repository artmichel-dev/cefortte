  ...
  return (
    <section ref={sectionRef} className={`bg-gray-200 ${className}`}>
      <article>
        {/* Header */}
        <div
          className={`container mx-auto text-center pb-12 transition-all duration-500 ease-out ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"
          }`}
        >
          <h3 className="text-display-lg uppercase text-left text-gray-800">{title}</h3>
          <p className="text-body pt-4 text-left text-gray-800">{subtitle}</p>
        </div>
        ...
        <div className="relative">
              ...
              {steps.map((step, index) => (
                ...
                >
                  ...
                  <div className="group flex-1 rounded-2xl bg-gray-100 p-6 hover:bg-white transition-colors">
                    ...
                    <b className="text-gray-800 group-hover:text-black transition-colors">{step.title}</b>
          ...
          <div className="hidden lg:grid lg:grid-cols-5 lg:gap-4 lg:auto-rows-fr">
            {steps.map((step, index) => (
              <div
                key={index}
                className={`group rounded-2xl bg-gray-100 p-6 hover:bg-white transition-colors h-full flex flex-col ${
                  isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"
                }`}
                style={{
              ...
              >
                ...
                <b className="text-gray-800 group-hover:text-black transition-colors">{step.title}</b>
                <p className="text-body pt-2 text-gray-800 flex-grow">{step.description}</p>
              </div>
            ))}
          </div>
        </div>
